...
export default async function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      <Link href="/events" className="inline-flex items-center gap-2 text-sm mb-8 transition-colors duration-200 text-[#8A8F98] hover:text-[#EDEDEF]">
        ...
        <div className="absolute top-4 left-4 flex items-center gap-2 flex-wrap">
          {e.is_recurring && e.recurrence_rule && (
            <span
              ...
              style={{ background: 'rgba(94,106,210,0.18)', color: '#818cf8', backdropFilter: 'blur(8px)', border: '1px solid rgba(94,106,210,0.30)' }}
            >
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold text-[#EDEDEF] tracking-tight">{e.title}</h1>
          {e.profiles && <p className="text-sm font-mono mt-1" style={{ color: '#8A8F98' }}>by @{e.profiles.username}</p>}
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          {isOwner && (
            <Link
              ...
              style={{ background: 'rgba(94,106,210,0.12)', color: '#818cf8', border: '1px solid rgba(94,106,210,0.25)' }}
            >
              Edit
            </Link>
          )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
        <DetailCard icon={<Clock className="w-4 h-4 text-[#5E6AD2]" />} label="Date & Time" value={format(new Date(e.start_time), 'EEE, MMM d yyyy · h:mm a')} />
        <DetailCard icon={<MapPin className="w-4 h-4 text-[#5E6AD2]" />} label="Location" value={e.location_name} />
        {e.end_time && <DetailCard icon={<Clock className="w-4 h-4 text-[#5E6AD2]" />} label="Ends" value={format(new Date(e.end_time), 'h:mm a')} />}
        <DetailCard icon={<Users className="w-4 h-4 text-[#5E6AD2]" />} label="Attending" value={`${attendeeCount} ${attendeeCount === 1 ? 'person' : 'people'}`} highlight />
        {e.min_rating_required && (
          <DetailCard icon={<Star className="w-4 h-4 text-[#5E6AD2]" />} label="Min. Rating" value={`${e.min_rating_required}+ stars recommended`} />
        )}
      </div>
      ...
      {maxCapacity !== null && (
        <div className="rounded-xl p-4 mb-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-mono uppercase tracking-widest" style={{ color: '#8A8F98' }}>Capacity</span>
            <span className="text-xs font-semibold" style={{ color: isFull ? '#f87171' : '#EDEDEF' }}>
          ...
          <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.08)' }}>
            <div
              ...
              style={{
                ...
                background: isFull ? '#ef4444' : '#5E6AD2',
              }}
            />
          </div>
        </div>
      ...
      {e.description && (
        <div className="rounded-2xl p-5 mb-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
          <div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-widest mb-3" style={{ color: '#8A8F98' }}>
            <FileText className="w-3.5 h-3.5 text-[#5E6AD2]" />
            About
          </div>
          <p className="text-sm text-[#EDEDEF] leading-relaxed whitespace-pre-wrap">{e.description}</p>
        </div>
      )}
      ...
      {e.organizer_contact && (
        <div className="rounded-2xl p-5 mb-6" style={{ background: 'rgba(34,197,94,0.04)', border: '1px solid rgba(34,197,94,0.18)' }}>
          ...
          <p className="text-sm text-[#EDEDEF]">{e.organizer_contact}</p>
        </div>
      )}
