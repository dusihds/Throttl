...
export default function NewEventPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const baseDate = tomorrow.toISOString().slice(0, 10)
    ...
    const { error: insertErr } = await supabase.from('car_events').insert({
      user_id:              user.id,
    ...
    <div className="max-w-xl mx-auto px-4 py-10">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="rounded-2xl p-5" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          {requiresPayment && (
            <p
              className="mt-4 text-xs px-3 py-2.5 rounded-lg"
              style={{ background: 'rgba(234,179,8,0.07)', color: '#fbbf24', border: '1px solid rgba(234,179,8,0.18)' }}
            >
              Attendees pay $1 to reserve their spot via Stripe. The fee is kept by the platform.
            </p>
          )}
        </div>
